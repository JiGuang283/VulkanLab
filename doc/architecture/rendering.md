...
开发运行时提供异步 PNG 截图，入口为 `Capture` 面板、F12 或 Runtime Control v3 的 `capture.screenshot/status/cancel`。Cooked package 当前不创建 CaptureService。截图根目录由 `ProjectContext::captureRoot` 提供；输出路径必须是该目录下的相对 `.png` 路径。
...
- Runtime Control 只创建、查询或取消任务；不会在服务端等待未来帧。`render.wait` 同样由控制客户端轮询 present progress 实现。
