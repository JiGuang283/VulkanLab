...
3. 在 `variants` 中登记稳定 ID、显示名、类别、Tone Mapping 策略和顺序。
4. 执行 Debug 构建。Manifest 变化会触发 CMake 重新配置，新增 Shader
   自动执行 `glslc`、`spirv-val` 和 runtime staging。
5. 运行 `ctest --test-dir build-debug -C Debug --output-on-failure`。
...
`id/name/category/toneMapping/default` 的 `entries`。`shader.set` 的现有字符串
