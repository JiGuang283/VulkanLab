...
std::vector<RenderImageUsage> SpotShadowPass::resourceUsages() const {
...
void SpotShadowPass::execute(const RenderFrameContext &frame,
    ...
    VKL_PROFILE_ZONE("Record SpotShadow");
    VKL_PROFILE_GPU_ZONE(*frame.tracyProfiler, frame.cmd, "SpotShadow");
    ...
    if (punctualData.activeSpotCount == 0)
        return;
    ...
    for (uint32_t lightIdx = 0;
         lightIdx < punctualData.activeSpotCount; ++lightIdx) {
        const auto &spot = punctualData.spots[lightIdx];
        if (!spot.enabled)
            continue;
        ...
        drawConfig.dynamicOffset =
            sliceBuffer_->dynamicOffset(lightIdx);
        ...
        ShadowCasterDrawRecorder::record(
            frame, visibility,
            visibility.spotShadowCasters[lightIdx], drawConfig);
...
void SpotShadowPass::createRenderPass(
    ...
    device_->debugUtils().setObjectName(
        ...
        "Pass/SpotShadow/RenderPass");
}

void SpotShadowPass::createFramebuffers(
    const RenderResourceRegistry &resources) {
    const VkFormat format =
        resources.description(shadowDepth_).format;
    const Image &image = resources.image(shadowDepth_, 0);
    for (uint32_t slot = 0; slot < kMaxSpotShadowLights; ++slot) {
            ...
            viewInfo.subresourceRange.baseArrayLayer = slot;
            ...
            fbInfo.pAttachments =
                &layerViews_[slot];
            fbInfo.width = kSpotShadowMapSize;
            fbInfo.height = kSpotShadowMapSize;
            fbInfo.layers = 1;
...
void SpotShadowPass::destroyFramebuffers() {
    for (uint32_t slot = 0; slot < kMaxSpotShadowLights; ++slot) {
            if (framebuffers_[slot] != VK_NULL_HANDLE) {
                vkDestroyFramebuffer(
                    device_->logicalDevice(),
                    framebuffers_[slot], nullptr);
                framebuffers_[slot] = VK_NULL_HANDLE;
            }
            if (layerViews_[slot] != VK_NULL_HANDLE) {
                vkDestroyImageView(
                    device_->logicalDevice(),
                    layerViews_[slot], nullptr);
                layerViews_[slot] = VK_NULL_HANDLE;
            }
    }
}
