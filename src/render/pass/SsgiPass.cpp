...
void SsgiPass::onViewportResize(const RenderResourceRegistry &resources) {
    createDescriptors(resources);
    status_.extent = resources.extent(resources_.ssgiFiltered);
}

void SsgiPass::execute(const RenderFrameContext &frame,
                       const RenderResourceRegistry &resources,
                       const VisibilityFrame &visibility) {
    status_.active = frame.features.ssgiActive;
    if (!frame.features.ssgiRequired || !frame.pipelineCache || !frame.view)
        return;
    VKL_PROFILE_ZONE("Record SSGI");
    VKL_PROFILE_GPU_ZONE(*frame.tracyProfiler, frame.cmd, "SSGI");
    ScopedGpuLabel label(device_->debugUtils(), frame.cmd, "SSGI");

    const uint32_t current = frame.frameIndex;
    const uint32_t previous =
        (current + resources.frameCount() - 1u) % resources.frameCount();
    if (!initialized_[previous]) {
        const std::array<RenderImageHandle, 6> previousOutputs = {
            resources_.ssgiRaw, resources_.ssgiHistory,
            resources_.ssgiMoments, resources_.ssgiTemp,
            resources_.ssgiFiltered, resources_.ssgiDebug};
        for (uint32_t index = 0; index < previousOutputs.size(); ++index) {
            const bool temp = index == 3;
            const Image &image = resources.image(previousOutputs[index],
                                                 previous);
            cmdImageBarrier(frame.cmd, VK_PIPELINE_STAGE_TOP_OF_PIPE_BIT,
                            VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT,
                            0u, VK_ACCESS_SHADER_READ_BIT,
                            image.handle(), VK_IMAGE_LAYOUT_UNDEFINED,
                            temp ? VK_IMAGE_LAYOUT_GENERAL
                                 : VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL,
                            colorRange());
        }
        initialized_[previous] = true;
    }
    ...
    if (!historyValid) {
        status_.lastResetReason =
            !visibility.history.invalidationReason.empty()
                ? visibility.history.invalidationReason
                : (!contiguous
                       ? "SSGI was not executed continuously"
                       : (!generationMatches
                              ? "temporal generation changed"
                              : (!settingsMatch ? "SSGI settings changed"
                                                : "history is unavailable")));
    } else {
        status_.lastResetReason.clear();
    }

    const std::array<RenderImageHandle, 6> outputs = {
        resources_.ssgiRaw, resources_.ssgiHistory,
        resources_.ssgiMoments, resources_.ssgiTemp,
        resources_.ssgiFiltered, resources_.ssgiDebug};
    for (uint32_t index = 0; index < outputs.size(); ++index) {
        const bool temp = index == 3;
        const Image &image = resources.image(outputs[index], current);
        cmdImageBarrier(
            frame.cmd,
    ...
    const VkPushConstantRange temporalRange{VK_SHADER_STAGE_COMPUTE_BIT, 0,
                                             sizeof(TemporalPush)};
    const VkPushConstantRange filterRange{VK_SHADER_STAGE_COMPUTE_BIT, 0,
                                           sizeof(FilterPush)};
    ComputePipelineConfig traceConfig{};
    traceConfig.debugName = "Pipeline/ScreenSpace/SSGI/Trace";
    ...
    temporalConfig.debugName = "Pipeline/ScreenSpace/SSGI/Temporal";
    ...
    ComputePipelineConfig filterConfig{};
    filterConfig.debugName = "Pipeline/ScreenSpace/SSGI/Filter";
    filterConfig.computeShaderPath = filterShaderPath_;
    filterConfig.descriptorLayouts = {filterLayout_};
    filterConfig.pushConstants = {filterRange};
    ...
    ComputePipeline &filter = frame.pipelineCache->getOrCreateCompute(
        std::move(filterConfig));
    ...
    const glm::uvec2 budget = samplingBudget(frame.view->settings.ssgiQuality);
    TracePush tracePush{};
    tracePush.parameters = {frame.view->settings.ssgiMaxDistance,
                            frame.view->settings.ssgiThickness,
                            frame.view->settings.ssgiIntensity,
                            frame.view->settings.ssgiRadianceClamp};
    tracePush.dimensions = {budget.x, budget.y, full.width, full.height};
    tracePush.sampling = {
    ...
    cmdImageBarrier(frame.cmd, VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT,
                    ...
                    resources.image(resources_.ssgiRaw, current).handle(),
                    VK_IMAGE_LAYOUT_GENERAL, VK_IMAGE_LAYOUT_GENERAL,
                    colorRange());

    TemporalPush temporalPush{};
    temporalPush.parameters = {frame.view->settings.ssgiHistoryWeight,
                               0.01f, 0.8f,
                               frame.view->settings.ssgiRadianceClamp};
    ...
    for (RenderImageHandle handle :
        ...
        cmdImageBarrier(frame.cmd, VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT,
                        ...
                        resources.image(handle, current).handle(),
                        VK_IMAGE_LAYOUT_GENERAL, VK_IMAGE_LAYOUT_GENERAL,
                        colorRange());
    }

    FilterPush filterPush{{full.width, full.height, 1u, 0u}};
    const auto dispatchFilter = [&](VkDescriptorSet set,
                                    uint32_t stepWidth) {
        filterPush.dimensions.z = stepWidth;
        vkCmdBindPipeline(frame.cmd, VK_PIPELINE_BIND_POINT_COMPUTE,
                          filter.handle());
        vkCmdBindDescriptorSets(frame.cmd, VK_PIPELINE_BIND_POINT_COMPUTE,
                                filter.layout(), 0, 1, &set, 0, nullptr);
        vkCmdPushConstants(frame.cmd, filter.layout(),
                           VK_SHADER_STAGE_COMPUTE_BIT, 0,
                           sizeof(filterPush), &filterPush);
        vkCmdDispatch(frame.cmd, groups(half.width), groups(half.height), 1);
    };
    dispatchFilter(firstFilterSets_[current], 1u);
    cmdImageBarrier(frame.cmd, VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT,
                    VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT,
                    VK_ACCESS_SHADER_WRITE_BIT, VK_ACCESS_SHADER_READ_BIT,
                    resources.image(resources_.ssgiTemp, current).handle(),
                    VK_IMAGE_LAYOUT_GENERAL, VK_IMAGE_LAYOUT_GENERAL,
                    colorRange());
    dispatchFilter(secondFilterSets_[current], 2u);
...
void SsgiPass::createDescriptors(const RenderResourceRegistry &registry) {
